...
def backtest_select_best_predictor(test_periods=300):
    """
    回测择优选择集成预测器
    ...
    """
    print("="*80)
    print(f"集成择优选择预测器 - {test_periods}期回测")
    print("="*80 + "\n")
    ...
    for name, data in sorted(stats.items()):
        ...
        print(f"  {display_name:20s}: {data['hits']}/{data['total']} = {data['rate']:.1f}%{current_mark}")
    
    # 统计使用次数
    predictor_usage = {}
    for r in results:
        pred = r['predictor_used']
    ...
    for pred, count in sorted(predictor_usage.items(), key=lambda x: x[1], reverse=True):
        ...
        print(f"  {pred:40s}: {count:3d}次 ({percentage:5.1f}%)")
    ...
    if max_consecutive_misses <= 6:
        ...
        improvement = ((12 - max_consecutive_misses) / 12) * 100
        print(f"  已改进: {improvement:.1f}% (从12期降至{max_consecutive_misses}期)")
    ...
    output_file = f'ensemble_select_best_{test_periods}periods.csv'
    ...
    if max_streak_periods:
        ...
        # 显示使用的预测器
        print(f"  使用的预测器:")
        for p in max_streak_periods:
            r = results[p - start_idx - 1]
            print(f"    第{p}期: {r['predictor_used']}")
...
if __name__ == '__main__':
    # 300期回测
    result = backtest_select_best_predictor(test_periods=300)
