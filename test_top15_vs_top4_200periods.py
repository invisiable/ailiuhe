...
print("="*120)
print("测试 TOP15 vs 生肖TOP4 对比分析 - 最近200期")
print("="*120)
print()
...
if len(df) < 200:
    print(f"❌ 数据不足200期，无法进行完整测试（当前仅{len(df)}期）")
    test_periods = len(df)
else:
    print(f"✓ 数据充足，可以测试完整200期")
    test_periods = 200
...
print(f"  • TOP15命中率: {top15_hit_rate:.2f}%")
print(f"  • TOP4命中率: {top4_hit_rate:.2f}%")
