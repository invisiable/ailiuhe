"""
混合组合策略 - 预测下一期验证
验证规则：使用当期（包含当天）的所有数据生成预测，与下一期的实际数字比对
"""
...
class HybridNextPeriodPredictor:
    """混合策略预测器 - 预测下一期"""
    ...
    def validate_next_period(self, csv_file, periods=10):
        """验证预测下一期的成功率"""
        print("=" * 80)
        print("混合组合策略 - 预测下一期验证")
        print("=" * 80)
        print(f"验证规则：")
        print(f"  1. 使用当期（包含当天）的所有数据生成预测")
        print(f"  2. 预测结果与下一期的实际数字进行比对")
        print(f"  3. 统计最近10期的预测成功率")
        print(f"\n策略说明：")
        ...
        for i in range(periods):
            current_index = total_records - periods - 1 + i
            next_index = current_index + 1
            
            if current_index < 10 or next_index >= total_records:
                continue
            
            # 使用当期（包含当天）的所有数据
            train_df = df.iloc[:next_index]  # 包含当期数据
            ...
            next_number = next_row['number']
            
            print(f"\n{'='*80}")
            print(f"验证第 {i+1}/{periods} 期")
            print(f"当期日期: {current_date}, 当期号码: {current_number}")
            print(f"预测目标: {next_date} (下一期)")
            print(f"使用数据: 截止到 {current_date} 的所有 {next_index} 期数据")
            ...
            if next_number in top5_predictions:
                rank = top5_predictions.index(next_number) + 1
                hit_level = "TOP 5"
                top5_success += 1
                top10_success += 1
                top15_success += 1
                status = f"✅ TOP 5 命中! (排名第 {rank})"
            elif next_number in top10_predictions:
                rank = top10_predictions.index(next_number) + 1
                hit_level = "TOP 10"
                top10_success += 1
                top15_success += 1
                status = f"✓ TOP 10 命中 (排名第 {rank})"
            elif next_number in top15_predictions:
                rank = top15_predictions.index(next_number) + 1
                hit_level = "TOP 15"
                top15_success += 1
        ...
        print("详细验证结果")
        print("=" * 80)
        print(f"{'期数':<6} {'当期日期':<12} {'预测日期':<12} {'实际':<6} {'命中':<10} {'排名':<6} {'状态':<15}")
        print("-" * 80)
        ...
        for r in results:
            ...
            print(f"{period_str:<6} {r['current_date']:<12} {r['next_date']:<12} {r['actual']:<6} {hit_level_str:<10} {rank_str:<6} {status_short:<15}")
...
def main():
    """主函数"""
    predictor = HybridNextPeriodPredictor()
    
    print("\n🔮 预测下一期验证模式")
    print("  ✓ 使用当期完整数据进行预测")
    print("  ✓ 预测结果与下一期实际数字比对")
    print("  ✓ 更符合实际使用场景\n")
    
    results = predictor.validate_next_period('data/lucky_numbers.csv', periods=10)
    
    print(f"\n{'='*80}")
    print("最终结论 - 混合组合策略（预测下一期）")
    print("="*80)
    ...
    print("="*80)
    print("\n本次验证采用【预测下一期】模式：")
    print("  - 使用当期（包含当天）的所有历史数据")
    print("  - 生成预测后与下一期实际数字比对")
    print("  - 这是实际应用中的真实预测场景")
    print("\n" + "="*80)
